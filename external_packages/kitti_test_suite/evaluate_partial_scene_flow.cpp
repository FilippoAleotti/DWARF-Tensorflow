...
#define FIRST_FRAME 160
#define NUM_TEST_IMAGES 200
#define NUM_ERROR_IMAGES 0
...
bool eval (string result_sha, string gt_folder, Mail* mail) {

  
  // ground truth and result directories
  string gt_img_dir = gt_folder + "image_2";
  string gt_obj_map_dir = gt_folder +"obj_map";
  string gt_disp_noc_0_dir = gt_folder +"disp_noc_0";
  string gt_disp_occ_0_dir = gt_folder +"disp_occ_0";
  string gt_disp_noc_1_dir = gt_folder +"disp_noc_1";
  string gt_disp_occ_1_dir = gt_folder +"disp_occ_1";
  string gt_flow_noc_dir = gt_folder +"flow_noc";
  string gt_flow_occ_dir = gt_folder +"flow_occ";
  string result_dir = result_sha;
  string result_disp_0_dir = result_dir + "/disp_0";
  string result_disp_1_dir = result_dir + "/disp_1";
  string result_flow_dir = result_dir + "/flow";

  // check availability of results
  bool avail_disp_0 = true;//resultsAvailable(result_disp_0_dir,"disp_0",mail);
  bool avail_disp_1 = true;//resultsAvailable(result_disp_1_dir,"disp_1",mail);
  bool avail_flow   = true;//resultsAvailable(result_flow_dir,"flow",mail);
  ...
  for (int32_t i=FIRST_FRAME; i<NUM_TEST_IMAGES; i++) {
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  if (argc!=2 && argc!=3 && argc!=4) {
    cout << "Usage: ./eval_scene_flow result_sha [gt_folder validity_mask]" << endl;
    return 1;
  }

  // read arguments
  string result_sha = argv[1];
  string gt_folder = "/media/faleotti/Storage/ComputerVision/Dataset/kitti_scene_flow/training/";

  if (argc==3){
    gt_folder = argv[2];
    char last_char = gt_folder.at(gt_folder.length() - 1 );
    if (last_char!='/')
      gt_folder += "/";
  }
  ...
  // run evaluation
  bool success = eval(result_sha,gt_folder, mail);
  return 0;
}
